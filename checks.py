...
def specClass_check_BPT(sample) :
    ...
    # print(totBPT)
    # totSey, totLIN, totComp, totSFG = np.sum(Sey), np.sum(LINER), np.sum(Comp), np.sum(SFG)
    # print(totSey, totLIN, totComp, totSFG)
    # print(totBPT - (totSey + totLIN + totComp + totSFG) )
    # not_ELG = (catalog['EmLineType'] == 'not_ELG') & (catalog['BPT'] == True)
    # print(np.sum(not_ELG))
    
    plt.diagram_BPT(log_NII_HA[Sey], log_OIII_HB[Sey],
                    log_NII_HA[LINER], log_OIII_HB[LINER],
                    log_NII_HA[Comp], log_OIII_HB[Comp],
                    log_NII_HA[SFG], log_OIII_HB[SFG])
...
def specClass_check_WHAN(sample) :
    ...
    # print(totWHAN)
    # totSey, totLIN, totComp, totSFG, totPass = np.sum(Sey), np.sum(LINER), np.sum(Comp), np.sum(SFG), np.sum(Pass)
    # print(totSey, totLIN, totComp, totSFG, totPass)
    # print(totWHAN - (totSey + totLIN + totComp + totSFG + totPass) )
    # not_ELG = (catalog['EmLineType'] == 'not_ELG') & (catalog['WHAN'] == True)
    # print(np.sum(not_ELG))
    
    plt.diagram_WHAN(log_NII_HA[Sey], HA_width[Sey],
                     log_NII_HA[LINER], HA_width[LINER],
                     log_NII_HA[Comp], HA_width[Comp],
                     log_NII_HA[SFG], HA_width[SFG],
                     log_NII_HA[Pass], HA_width[Pass])
