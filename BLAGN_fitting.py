...
# directory = 'CARS_DR7_spectra/named/'
    ...
    for file in files :        
        ...
        if HA_SN >= 200 :
            ...
            HA_NII_core = ( continuum/k_act +
                           1.00*gauss(wave[select], amp_Ha_core, vel_Ha_core,
                                      vel_sigma_Ha_core, 6562.85, 2.5) +
                           0.33*gauss(wave[select], amp_NII6583_core, vel_Ha_core,
                                      vel_sigma_Ha_core, 6548.05, 2.5) +
                           1.00*gauss(wave[select], amp_NII6583_core, vel_Ha_core,
                                      vel_sigma_Ha_core, 6583.45, 2.5) )
            # HA_NII_wing = ( continuum/k_act +
            #                1.00*gauss(wave[select], amp_Ha_wing, vel_Ha_wing,
            #                           vel_sigma_Ha_wing, 6562.85, 2.5) +
            #                0.33*gauss(wave[select], amp_NII6583_wing, vel_Ha_wing,
            #                           vel_sigma_Ha_wing, 6548.05, 2.5) +
            #                1.00*gauss(wave[select], amp_NII6583_wing, vel_Ha_wing,
            ...
            plt.plot_decoupled_fit(file, x_array, cs_spec[select], fit, residual,
                                   HA_NII_core, BLR1)
            
    # plt.histo(SNs, r'H$\rm \alpha$ SN', 'BLAGN Candidates')
...
def complex_gauss_decoupled(input_vals, wave, data, error) :
    ...
    HA_NII_core = (1.00*gauss(wave, amp_Ha_core, vel_Ha_core,
                              vel_sigma_Ha_core, 6562.85, 2.5) +
                   0.33*gauss(wave, amp_NII6583_core, vel_Ha_core,
                              vel_sigma_Ha_core, 6548.05, 2.5) +
                   1.00*gauss(wave, amp_NII6583_core, vel_Ha_core,
                              vel_sigma_Ha_core, 6583.45, 2.5) )
    # HA_NII_wing = (1.00*gauss(wave, amp_Ha_wing, vel_Ha_wing,
    #                           vel_sigma_Ha_wing, 6562.85, 2.5) +
    #                0.33*gauss(wave, amp_NII6583_wing, vel_Ha_wing,
    #                           vel_sigma_Ha_wing, 6548.05, 2.5) +
    #                1.00*gauss(wave, amp_NII6583_wing, vel_Ha_wing,
    #                           vel_sigma_Ha_wing, 6583.45, 2.5) )
