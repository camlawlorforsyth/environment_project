...
def CATALOG_settings():
    ...
    ***USER INPUT NEEDED***
    ...
    ##GENERAL
    cat['path'] ='/home/cam/Downloads/AGNfitter/'  #path to the AGNfitter code


    cat['filename'] = cat['path']+'data/catalog_test.txt'
    ...
    cat['freq/wl_list'] = np.arange(2,44,3).tolist()                                  
    ...
    cat['flux_list'] = np.arange(3,45,3).tolist()        
                                        ## If ASCII: List of column indexes (int)
    #cat['flux_suffix'] = '_f'          ## If FITS: Common ending of all flux column names (str)
    
    cat['fluxerr_list'] = np.arange(4,46,3).tolist() 
                                        ## If ASCII: List of column indexes (int)
...
def FILTERS_settings():
    ...
    filters['dict_zarray'] =np.array([0.042032, 0.042396, 0.037953, 0.042065, 0.041923])  # The grid of redshifts needed to fit your catalog
